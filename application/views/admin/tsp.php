...
######## ÇAPRAZLAMA BAŞLANGIÇ ########
	...
	for($k=0;$k<$caprazlama;$k++)
	{
		...
		$random1 = rand(0,$populasyon-1);
		$random2 = rand(0,$populasyon-1);
...
#### MUTASYON ####
	...
	for($i=0;$i<$mutasyon;$i++)
	{
		
		$randomkromozom = rand(0,$populasyon-1);
		$randomgen1 = rand(0,6);
		$randomgen2 = rand(0,6);
	
		$tmp 	= 	str_split($islemkromozom[$randomkromozom]['KromozomAdi']);
		
		$kap = $tmp[$randomgen1];
		$tmp[$randomgen1] = $tmp[$randomgen2];
		$tmp[$randomgen2] = $kap;
	
		$tmp[0] = implode('',$tmp);
		$abc[0]['KromozomAdi'] = $tmp[0];
		$abc[0]['Uzaklik'] = UzaklikHesapla($tmp[0], $mesafeler);
		
		if($abc[0]['Uzaklik']<$islemkromozom[$randomkromozom]['Uzaklik']){
		$islemkromozom[$random1]['Uzaklik']=$abc[0]['Uzaklik'];
		$islemkromozom[$random1]['KromozomAdi']=$abc[0]['KromozomAdi'];
	}	
	
		echo "MUTASYONA UGRAYAN:",$islemkromozom[$randomkromozom]['KromozomAdi']," ","MUTASYON SONUCU: "," ",$abc[0]['KromozomAdi'],"<br>";
		echo "MUTASYONA UGRAYAN:",$islemkromozom[$randomkromozom]['Uzaklik']," ","MUTASYON SONUCU: "," ",$abc[0]['Uzaklik'],"<br>";
	}
	
#### MUTASYON ####

		$sonkromozom[$x]['KromozomAdi'] = $islemkromozom[0]['KromozomAdi'];
		$sonkromozom[$x]['Uzaklik'] = $islemkromozom[0]['Uzaklik'];
		
		echo "Bu populasyondaki en kısa kromozom:",$sonkromozom[$x]['KromozomAdi']," ","Uzaklik = "," ",$sonkromozom[$x]['Uzaklik'],"<br>";	
}
	
		for($i = 0; $i < 100; $i++) 
		{	
			for($j = 0; $j < 100; $j++){
				if($sonkromozom[$i]['Uzaklik'] < $sonkromozom[$j]['Uzaklik'])
				{
				
				$tmp = $sonkromozom[$i]['Uzaklik'];
				$sonkromozom[$i]['Uzaklik'] = $sonkromozom[$j]['Uzaklik'];
				$sonkromozom[$j]['Uzaklik'] = $tmp;
						
				$tmp2 = $sonkromozom[$i]['KromozomAdi'];
				$sonkromozom[$i]['KromozomAdi'] = $sonkromozom[$j]['KromozomAdi'];
				$sonkromozom[$j]['KromozomAdi'] = $tmp2;						
				}
										
			}
		}
			echo "En Kısa Kromozom:",$sonkromozom[0]['KromozomAdi']," ","Uzaklik = "," ",$sonkromozom[0]['Uzaklik'],"<br>";	
...
## Eksik kalanlar. Çaprazlamada en iyiler çaprazlanmadı. Çaprazlama sonucu çocuk ile ebeveyn değişim kontrolü yapılmadı. Mutasyon yapılmadı.
## Jenerasyonlardan sonra oluşan en iyiler bir değişkende tutulup aralarında sıralanıp en iyi seçimi yapılmadı.